...
export async function POST(request: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const salon = await tx.salon.create({ data: { name: salonName, address: String(body.address || ''), timezone: 'Europe/Berlin', locale: 'de-DE', subscriptionStatus: 'TRIALING', greetingText: `Hallo! Ich bin die digitale Assistentin von ${salonName}.` } });
    ...
    return NextResponse.json({ ok: true, salon: result.salon, user: { id: result.user.id, email: result.user.email, role: result.user.role } }, { headers: { 'Set-Cookie': makeSessionCookie({ userId: result.user.id, salonId: result.salon.id, role: result.user.role, email: result.user.email }) } });
  } catch (error) {
